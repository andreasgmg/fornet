...
import { Snowflake, Droplets, MapPin, Calendar, ArrowRight, FileText, Key, AlertTriangle, ChevronRight, Info, Recycle, Briefcase } from 'lucide-react';
...
export default async function TenantPage({ params }: { params: Promise<{ site: string }> }) {
  ...
  return (
    <main className="pb-20 bg-slate-50 min-h-screen">
      ...
      {org.alert_message && (
          <div className="bg-yellow-100 text-yellow-800 px-4 py-3 flex items-center justify-center gap-2 text-sm font-bold shadow-sm relative z-20">
      ...
      <header className="relative bg-gray-900 text-white overflow-hidden pb-12 md:pb-32 -mx-4 md:-mx-6 rounded-b-[1.5rem] md:rounded-b-[3rem] shadow-lg mb-6 md:mb-12">
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-6 pt-20 md:pt-32 text-center md:text-left">
            ...
            <p className="text-sm md:text-lg text-gray-200 max-w-xl leading-relaxed font-medium hidden md:block">
                Här hittar du nyheter, bokning och driftinformation för oss som bor i området.
            </p>
            <p className="text-sm text-gray-300 max-w-xl leading-relaxed font-medium md:hidden opacity-90">
                Din digitala föreningslokal.
            </p>
        </div>
      </header>
      ...
      <div className="max-w-6xl mx-auto px-3 md:px-6 relative z-20 -mt-8 md:-mt-24 mb-8 md:mb-16">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
            {config.show_booking && <QuickAction icon={Key} title="Boka tid" href="/boka" colorClass="bg-blue-50 text-blue-600" />}
            {config.show_documents && <QuickAction icon={FileText} title="Dokument" href="/dokument" colorClass="bg-purple-50 text-purple-600" />}
            {config.show_contact_widget && <QuickAction icon={AlertTriangle} title="Felanmälan" href="/felanmalan" colorClass="bg-orange-50 text-orange-600" />}
            {config.show_calendar_qlink && <QuickAction icon={Calendar} title="Kalender" href="/kalender" colorClass="bg-green-50 text-green-600" />}
            {config.show_waste_sorting && <QuickAction icon={Recycle} title="Sopsortering" href="/s/sopsortering" colorClass="bg-green-50 text-green-600" />}
            {config.show_broker_info && <QuickAction icon={Briefcase} title="Mäklarinfo" href="/maklarinfo" colorClass="bg-gray-100 text-gray-700" />}
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-3 md:px-6 grid grid-cols-1 lg:grid-cols-3 gap-8 md:gap-12">
        ...
        <aside className="space-y-6 md:space-y-8">
            ...
            {config.show_calendar_widget && (
                <div id="kalender" className="bg-white border border-slate-200 p-5 md:p-6 rounded-2xl shadow-sm md:sticky md:top-24">
